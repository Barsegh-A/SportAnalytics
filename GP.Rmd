...
data1 <- data1 %>% 
  ...
  rename(ATP = Position)
data1 <- data1 %>% mutate(Surprise_Points = ifelse((FTR == 'A' & HTP < ATP) | (FTR == 'H' & HTP > ATP), 2,
                                         ifelse(FTR == 'D' , 1, 0)))
```

```{r}
sp <- data1 %>% group_by(SEASON, COUNTRY) %>% summarize(Surprise_Points = sum(Surprise_Points))
...
HHI <- f %>% group_by(SEASON, COUNTRY) %>%
  ...
  summarise(HHI = sum(Perc^2))
HHI <- HHI %>% 
  left_join(dplyr::select(sp, SEASON, COUNTRY, S, N), by = c("SEASON" = "SEASON", "COUNTRY" = "COUNTRY"))
```
