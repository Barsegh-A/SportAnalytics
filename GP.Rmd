...
```{r}
libs<-c("caret","tidyr", "ggplot2", "knitr", "dplyr", "purrr", "MASS", "BradleyTerry2", "devtools", "reldist", "shiny")
...
renderPlot({
  ...
  ggplot() +
    geom_point(data = d, aes_string(x = "SEASON", y = (input$metric))) +
      geom_line(data = d, aes_string(x = "SEASON", y = (input$metric), color = "LEAGUE")) +
    geom_point(data = d, aes_string(x = "SEASON", y = (input$metric))) +
      geom_line(data = d, aes_string(x = "SEASON", y = (input$metric), color = "LEAGUE")) +
    labs(x = "Season",
         y = "Home Team Advantage",
         title = "Home Team Advatage per Season by League")
})
```
